...
from evaluator.evaluator import StockFishEvaluator
from constants import STOCKFISH_CONFIG, DEPTH
eval = StockFishEvaluator(STOCKFISH_CONFIG, DEPTH)
  ...
  def collect_trajectories():
    ...
    for trajectory in trajectory_generator():
      ...
      for state in trajectory.states:
        p1_outcome = eval.evaluate(str(state))
        if p1_outcome > 0:
          outcomes.add(0)
        elif p1_outcome < 0:
          outcomes.add(1)
          ...
          outcomes.add(2)
        returns.append(p1_outcome)
      replay_buffer.extend(
          model_lib.TrainInput(
              s.observation, s.legals_mask, s.policy, outcome)
          for s, outcome in zip(trajectory.states, returns))
